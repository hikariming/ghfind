...
function parseBoard(value: unknown): ProjectBoard {
  ...
  return scalar === "classic" ? "classic" : "treasure";
}
...
export default async function ProjectsPage({
  ...
  try {
    ...
    if (error instanceof ProjectAnalysisDatabaseError) databaseError = error.message;
    else throw error;
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col px-5 py-14 sm:px-6 sm:py-20">
      ...
      <nav className="mt-8 flex gap-2" aria-label={t("boardLabel")}>
        {(["treasure", "classic"] as const).map((value) => (
          <Link
            key={value}
            href={value === "treasure" ? "/projects" : "/projects?board=classic"}
