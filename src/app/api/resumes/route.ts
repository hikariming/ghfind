...
import { readResumeLibrary, sameResumeContent, resumeSchema, serializeResumeLibrary, upsertResume, RESUME_LIBRARY_MAX_BYTES } from "@/lib/resume";
...
export async function PUT(request: NextRequest) {
  ...
  const parsed = resumeSchema.safeParse(payload?.resume);
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (row === undefined) return jsonNoStore({ error: "resumes_unavailable" }, { status: 503 });
    let library;
    try { library = readResumeLibrary(row?.data ?? null); }
    catch { return jsonNoStore({ error: "resumes_unavailable" }, { status: 503 }); }
    const previous = library.find(item => item.id === parsed.data.id);
    ...
    let data;
    try { data = serializeResumeLibrary(upsertResume(library, resume)); }
