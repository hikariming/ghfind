...
1. **定性复核**：阅读 top_repos 的 readme_excerpt、recent_prs、**impact_repos**（该账号长期向其贡献过 PR/commit 的高星仓库，含较早的工作，如 apache/flink 等）、**verified_impact_prs**（已抓到文件路径的高星 PR 样本）与 **flood_pr_titles**（近期 PR 标题样本），发现公式抓不到的信号（模板/AI 生成仓库、awesome-list 凑 star、水 PR、**模板化 PR 洪水/AI 批量刷 PR**、或被低估的真实利基专家），据此决定上面的 delta。注意：**recent_prs 只是最近 merged PR 样本，不代表全部 PR 分布；不能从 recent_prs 推断"全部/所有/N 个 PR 都在某类仓库"。全量高星外部贡献看 impact_repos / impact_pr_count。** recent_doc_like_pr_ratio 包含自有仓库，判断外部贡献质量时优先看 recent_external_doc_like_pr_ratio 与 verified_impact_prs 的 core/doc-like 拆分。原创项目 star 只有在最高星仓库本身像可用项目时才值得认可；若 top_starred_original_repo_quality_score 很低，不要因为 star 给正向 delta。metrics.impact_prs_outside_quality_sample 只表示部分长期高星贡献没有文件级样本，**不是负面指标，不能单独作为扣分依据**。若 metrics.impact_quality_cap 存在，说明高星生态影响主要来自文档/站点/示例/模板或 contribution graph 未验证归因，**不得用正向 delta 把最终分抬到 60 以上**；若同时 recent_external_doc_like_pr_ratio >= 0.55 且 top_starred_original_repo_quality_score < 0.3，\`@@ADJUST\` 不得为正。若 core_impact_pr_count 很少且 doc_like_impact_pr_count 更多，报告中不要写“核心贡献/核心修复/深入核心/Committer”，应如实写“站点/文档/示例/模板/前端界面类贡献为主”。**不要因为给 Apache 等组织仓库提过 PR 就推断其是 Committer，除非输入数据明确给出该身份。** 若 flood_pr_titles 明显是同一模板批量生成（如一天刷十几个「migrate ___ to X」），应**下调** delta。**绝不**把已命中的硬性 red flag（如 follow_farming、trivial_pr_farming、templated_pr_flooding）洗成高等级。但**给自己仓库提的 PR 一律不算刷量**，别因为某人主要在自己项目上提交就压低 delta。
...
| 生态/维护影响力 | x/20 | 向 ★… 仓库贡献 N 个实质 PR/commit(综合长期贡献，见 impact_repos；如确有自有热门项目再注明) |
| 社区影响力 | x/8 | followers … |
| 活跃真实性 | x/17 | 近一年贡献 … |

**风险标记**: <逐条列出 red_flags 及细节，或"无">
**人工修正**: <与开头 @@ADJUST@@ 一致的 ±N 及理由，或"无（0）">
**建议**: <如 优先处理 / 正常 / 需人工复核 / 疑似机器人建议拦截；若 context_notes.required_verdict 存在，必须使用该建议>
\`\`\`
...
1. **Qualitative review**: read the top_repos readme_excerpt, recent_prs, **impact_repos** (popular repos this account has contributed PRs/commits to over time, including older work such as apache/flink), **verified_impact_prs** (popular-repo PR samples with file paths), and **flood_pr_titles** (a sample of recent PR titles) to spot signals the formula misses (templated/AI-generated repos, awesome-list star padding, trivial PRs, **templated PR floods / AI bulk-spammed PRs**, or an underrated genuine niche expert), and decide the delta above. Note: **recent_prs is only a recent merged-PR sample, not the full PR distribution; never infer "all/every/N PRs are in one repo/type" from recent_prs. All-time popular external contributions live in impact_repos / impact_pr_count.** recent_doc_like_pr_ratio includes own repos; for external-contribution quality, prefer recent_external_doc_like_pr_ratio and verified_impact_prs core/doc-like counts. Original-project stars only deserve praise when the top-starred repo itself looks like a usable project; if top_starred_original_repo_quality_score is low, do not give positive delta for those stars. metrics.impact_prs_outside_quality_sample only means some all-time popular-repo contributions do not have file-level samples in this prompt; **it is not a negative metric and must not be used alone as a reason to lower the score**. If metrics.impact_quality_cap is present, the high-star ecosystem signal is mostly docs/site/examples/templates or unverified contribution-graph attribution; **do not use a positive delta to lift the final score above 60**; if recent_external_doc_like_pr_ratio >= 0.55 and top_starred_original_repo_quality_score < 0.3, \`@@ADJUST\` must not be positive. If core_impact_pr_count is small and doc_like_impact_pr_count is larger, do not call the work "core engineering", "core contribution", or "core fix" — describe it as docs/site/examples/templates/frontend UI work. **Do not infer titles such as Apache Committer merely from PRs to Apache repos unless the input explicitly says so.** If flood_pr_titles are clearly one template mass-produced (e.g. a dozen "migrate ___ to X" in a day), you should **lower** the delta. **Never** launder an already-triggered hard red flag (follow_farming, trivial_pr_farming, templated_pr_flooding) into a high tier. But **PRs to one's own repos never count as farming** — don't lower the delta just because someone mostly commits to their own projects.
...
| Ecosystem / maintenance impact | x/20 | N substantive PRs/commits into ★… repos (all-time, see impact_repos; mention own popular projects only when applicable) |
...
**Verdict**: <e.g. prioritize / normal / needs human review / likely bot, recommend blocking; if context_notes.required_verdict exists, use it>
\`\`\`
