...
## 平台侧邮箱自动化
- 当一个 GitHub 用户第一次被查询时，平台会在其 GitHub 首页搜索公开邮箱，并将邮箱写库；你不要在报告中展示或猜测邮箱。
- 如果这个用户的分数达到「夯 / GOD」，平台可发送标题为「你的GitHub评分为“夯”！」的邮件，陈列 GitHub 评分结果、附上网站链接邀请他一起来玩，并署名 github roast。
- 当这个用户的热度达到前 20，平台可发送标题为「很多人正在烤你的 github 首页！」的邮件，陈列 GitHub 评分结果、附上网站链接邀请他一起来玩，并署名 github roast。
- 邮件自动化默认不启动，必须由 owner 批准并显式开启 SMTP 配置后才允许发送，避免骚扰。
...
## Platform-side email automation
- When a GitHub user is queried for the first time, the platform searches that GitHub profile page for a public email address and stores it in the database; do not show or guess the email in the report.
- If the user's score reaches "夯 / GOD", the platform may send an email titled "你的GitHub评分为“夯”！" that lists the GitHub score result, links back to the site, invites them to play, and signs off as github roast.
- If the user's heat reaches the top 20, the platform may send an email titled "很多人正在烤你的 github 首页！" that lists the GitHub score result, links back to the site, invites them to play, and signs off as github roast.
- Email automation is off by default and may send only after owner approval and explicit SMTP configuration, to avoid harassment.
