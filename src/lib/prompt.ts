...
- closed PR 口径：只有 maintainer_closed_unmerged_pr_count 才能称为"被维护者拒绝/关闭"；self_closed_external_pr_count 和 self_closed_own_repo_pr_count 是作者主动关闭，不要写成被拒。写"通过率/被拒率"时优先使用 pr_rejection_rate 与这些拆分字段，别直接把 closed_unmerged_pr_count 当 rejected。
...
- Closed PR semantics: only maintainer_closed_unmerged_pr_count is maintainer rejection/closure. self_closed_external_pr_count and self_closed_own_repo_pr_count were closed by the author; do not describe them as rejected. When writing acceptance/rejection rates, use pr_rejection_rate and these split fields instead of treating closed_unmerged_pr_count as rejected.
