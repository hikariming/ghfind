...
1. **定性复核**：阅读 top_repos 的 readme_excerpt、recent_prs、**impact_repos**（该账号长期向其贡献过 PR/commit 的高星仓库，含较早的工作，如 apache/flink 等）与 **flood_pr_titles**（近期 PR 标题样本），发现公式抓不到的信号（模板/AI 生成仓库、awesome-list 凑 star、水 PR、**模板化 PR 洪水/AI 批量刷 PR**、或被低估的真实利基专家），据此决定上面的 delta。注意：**recent_prs 只是最近 merged PR 样本，不代表全部 PR 分布；不能从 recent_prs 推断"全部/所有/N 个 PR 都在某类仓库"。全量高星外部贡献看 impact_repos / impact_pr_count。** 若 flood_pr_titles 明显是同一模板批量生成（如一天刷十几个「migrate ___ to X」），应**下调** delta。**绝不**把已命中的硬性 red flag（如 follow_farming、trivial_pr_farming、templated_pr_flooding）洗成高等级。但**给自己仓库提的 PR 一律不算刷量**，别因为某人主要在自己项目上提交就压低 delta。
...
1. **Qualitative review**: read the top_repos readme_excerpt, recent_prs, **impact_repos** (popular repos this account has contributed PRs/commits to over time, including older work such as apache/flink) and **flood_pr_titles** (a sample of recent PR titles) to spot signals the formula misses (templated/AI-generated repos, awesome-list star padding, trivial PRs, **templated PR floods / AI bulk-spammed PRs**, or an underrated genuine niche expert), and decide the delta above. Note: **recent_prs is only a recent merged-PR sample, not the full PR distribution; never infer "all/every/N PRs are in one repo/type" from recent_prs. All-time popular external contributions live in impact_repos / impact_pr_count.** If flood_pr_titles are clearly one template mass-produced (e.g. a dozen "migrate ___ to X" in a day), you should **lower** the delta. **Never** launder an already-triggered hard red flag (follow_farming, trivial_pr_farming, templated_pr_flooding) into a high tier. But **PRs to one's own repos never count as farming** — don't lower the delta just because someone mostly commits to their own projects.
