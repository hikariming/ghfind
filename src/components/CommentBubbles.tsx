...
export interface CommentBubbleItem {
  id: string;
...
const ANONYMOUS_LABEL: Record<CommentBubbleLang, string> = {
  zh: "匿名",
  en: "Anonymous",
};

const COMMENT_LABELS: Record<CommentBubbleLang, CommentBubbleLabels> = {
  zh: {
    anonymous: "匿名发布",
    anonymousActive: "已匿名",
...
const MOBILE_DANMAKU_TOPS = ["0.5rem", "3rem", "5.5rem", "8rem", "10.5rem", "13rem", "15.5rem"];
const MOBILE_DANMAKU_STAGGER_SECONDS = 0.5;
const MOBILE_DANMAKU_LOOP_GAP_SECONDS = 5;

function bubbleStyle(
  bubble: FloatingCommentBubble,
  contentHalfWidth: string,
): CSSProperties {
  const laneOffset = `min(${bubble.laneOffset}, 5vw)`;
  const sideOffset = `calc(50% + ${contentHalfWidth} + 1rem + ${laneOffset})`;

  return {
    top: bubble.top,
    maxWidth: `min(14rem, calc(50vw - ${contentHalfWidth} - 2rem - ${laneOffset}))`,
...
function layoutBubble(author: CommentAuthor, text: string, index: number): FloatingCommentBubble {
  return {
...
function githubAvatarUrl(author: Extract<CommentAuthor, { type: "github" }>) {
...
export function CommentBubbles({
  ...
  contentHalfWidth: string;
  initialComments?: CommentBubbleItem[];
  lang: CommentBubbleLang;
  loadOnMount?: boolean;
}) {
  ...
  useEffect(() => {
    ...
    void fetch(commentsEndpoint, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) return null;
        return (await response.json()) as { comments?: CommentBubbleItem[] };
      })
      .then((payload) => {
  ...
  }, [commentsEndpoint, loadOnMount]);

  const visibleComments = [...comments].reverse();
  const bubbles = visibleComments.map((comment, index) =>
    layoutBubble(comment.author, comment.text, index),
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = (await response.json()) as { comment: CommentBubbleItem };
    ...
    <>
      <div className="pointer-events-none absolute inset-0 z-0 hidden overflow-hidden xl:block">
        {bubbles.map((bubble, index) => (
          <div
            key={`${bubble.side}-${index}-${bubble.text}`}
            className="floating-comment-bubble absolute w-max min-w-0 max-w-[14rem]"
            style={bubbleStyle(bubble, contentHalfWidth)}
          >
        ...
        <div className="pointer-events-none fixed inset-x-0 top-16 z-20 h-72 overflow-hidden xl:hidden">
          {bubbles.map((bubble, index) => (
            <div
              ...
              style={mobileDanmakuStyle(index, bubbles.length)}
            >
