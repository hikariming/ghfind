  ...
  return <main className={`resume-app ${view === "editor" ? "resume-is-editing" : ""}`}>
    ...
    {!ready ? <p className="resume-loading">{copy("正在读取本地简历…", "Reading local résumés…")}</p> : view === "gallery" ? <>
      <section className="resume-cloud" aria-label={copy("保存与同步", "Storage & sync")}>
        <div className="resume-cloud-info"><h2>{me?.user ? copy(`@${me.user.login} 的简历`, `@${me.user.login}'s résumés`) : copy("在不同设备上继续编辑", "Continue on any device")}</h2><p>{cloudState === "loading" ? copy("正在读取云端简历…", "Loading cloud résumés…") : cloudState === "error" ? copy("暂时无法读取云端，本地简历仍可编辑。", "Cloud unavailable. Your local résumés are still here.") : cloudState === "signed-out" ? me?.oauth === false ? copy("此环境暂未开启登录，请先保存到此浏览器。", "Sign-in is unavailable here. Save in this browser for now.") : copy("登录 GitHub，将需要的简历保存到你的账号。", "Sign in with GitHub to save résumés to your account.") : copy("云端内容已读取。点击简历即可编辑，无需手动加载或覆盖本地库。", "Cloud library loaded. Open a résumé to edit; local drafts stay untouched.")}</p></div>
      ...
      {cloudState === "ready" && cloudLibrary.length === 0 && <p className="resume-empty">{copy("还没有云端简历。打开本地简历或选择模板，编辑后点击「保存到云端」。", "No cloud résumés yet. Open a local draft or choose a template, then save to the cloud.")}</p>}
