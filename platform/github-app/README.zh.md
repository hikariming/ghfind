...
| 现象               | 排查方式                                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------------------- |
| 没有标签或评论     | 确认 App 已安装到该仓库并已接受完整权限。安装时会给当时 Open 的 issue 和 PR 排队，数量多时逐页处理。从安装流程或安装设置进入状态页查看任务。 |
...
每位作者跨仓库每 48 小时最多一封，全局每个 UTC 日最多 100 次。同一作者在同一仓库的多条 issue 或 PR 只写一条评分评论。非 GitHub App 令牌额度问题引起的 `review: no-score`，每个仓库最多带评论打 3 次，之后只打标签、不再评论。
