...
async function main() {
  ...
  try {
    ...
    const recoverAssessment = async created => {
      ...
      const recover = async (secret, expected, request = command) => {
        const response = await fetch(`${origin}/api/internal/project-analyses/reconcile`, { method: 'POST', headers: { authorization: `Bearer ${secret}`, 'content-type': 'application/json' }, body: JSON.stringify(request) });
      ...
      const interrupted = await control('/interrupt-recovery');
      const failedRetry = await browserCall(page, 'GET', created.statusUrl);
      assert.equal(failedRetry.status, 'failed');
      assert.equal(failedRetry.error.code, 'mosoo_run_failed');
      const finalCommand = { ...command, action: 'retry_interrupted_final', requestId: randomUUID(), expectedThreadId: interrupted.threadId, expectedRunId: interrupted.runId };
      const finalRecovered = await recover(process.env.PROJECT_ANALYSIS_RECONCILE_SECRET, 200, finalCommand);
      assert.equal(finalRecovered.analysisId, created.analysisId);
      assert.equal(finalRecovered.startedAt, original.startedAt);
      assert.equal(finalRecovered.idempotencyKey, `${original.idempotencyKey}-retry-2`);
      const finalBeforeReplay = await control('/recovery-inspect');
      await recover(process.env.PROJECT_ANALYSIS_RECONCILE_SECRET, 200, finalCommand);
      const finalAfterReplay = await control('/recovery-inspect');
      assert.equal(finalAfterReplay.logicalAnalyses, 1);
      assert.equal(finalAfterReplay.providerAttempts, 3);
      assert.deepEqual(finalAfterReplay, finalBeforeReplay, 'final recovery replay mutated state, audit, or provider attempts');
      assert.deepEqual(finalAfterReplay.audit, afterReplay.audit, 'final recovery rewrote the prior immutable audit');
      assert.equal(finalAfterReplay.run.started_at, original.startedAt);
      assert.equal(finalAfterReplay.finalAudit.length, 1);
      const finalAudit = finalAfterReplay.finalAudit[0];
      assert.equal(finalAudit.request_id, finalCommand.requestId);
      assert.equal(finalAudit.predecessor_request_id, command.requestId);
      assert.equal(finalAudit.original_thread_id, interrupted.threadId);
      assert.equal(finalAudit.original_run_id, interrupted.runId);
      assert.equal(finalAudit.original_started_at, original.startedAt);
      assert.equal(finalAudit.original_error_code, 'mosoo_run_failed');
      assert.equal(finalAudit.execution_deadline_at - finalAudit.requested_at, 30 * 60_000);
      assert.equal(finalAudit.next_idempotency_key, finalRecovered.idempotencyKey);
      await recover(process.env.PROJECT_ANALYSIS_RECONCILE_SECRET, 409, command);
      await recover(process.env.PROJECT_ANALYSIS_RECONCILE_SECRET, 409, { ...finalCommand, requestId: randomUUID(), expectedThreadId: finalRecovered.threadId, expectedRunId: finalRecovered.runId });
      assert.deepEqual(await control('/recovery-inspect'), finalAfterReplay, 'rejected third recovery changed durable state or provider attempts');
      report.milestones.assessmentOperatorRecovery = true;
      report.operatorRecovery = { status: 'passed', fault: original.fault, logicalAnalyses: 1, providerAttempts: 3, auditRows: 2, originalStartedAtPreserved: true, priorAuditUnchanged: true, replayCreatedAttempt: false, thirdRetryRejected: true };
    };
    ...
    assert.equal(report.result.source.providers.assessmentCreate, 5);
