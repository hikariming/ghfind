...
try:
 ...
 for name,origin,service in [(API,'http://127.0.0.1:58085','feed-api'),(WORKER,'http://127.0.0.1:58086','feed-worker')]:
  ...
  probe=REPORT['checks'][name+'-initial-probe'];assert probe['uid']==65532 and probe['gid']==65532 and probe['rootWriteReadOnly'] and probe['tmpWriteRead'] and not probe['markerExistedBefore'],probe
 ...
 run(['docker','cp',API+':/usr/local/bin/feed-api',str(OUT/'feed-api.elf')]);run(['docker','cp',WORKER+':/usr/local/bin/feed-worker',str(OUT/'feed-worker.elf')])
 REPORT['binaries']={name:{'sha256':hashlib.sha256((OUT/(name+'.elf')).read_bytes()).hexdigest(),'buildInfo':run(['go','version','-m',str(OUT/(name+'.elf'))]).stdout} for name in ['feed-api','feed-worker']}
