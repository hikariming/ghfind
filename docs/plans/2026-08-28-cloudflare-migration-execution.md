...
>
> **状态（2026-08-30）**：本计划的 Worker、路由回迁与 D1 切换已合入
> `main`。以下未勾选项保留为迁移时的历史记录，不得作为当前部署指令；
> 当前操作以 `wrangler.jsonc`、
> [Cloudflare Worker runbook](../operations/cloudflare-worker-runbook.md) 和
> [deployment smoke](../releases/deployment-smoke.md) 为准。
...
- 2026-08-30：**仓库与发布流程对齐**：Cloudflare 迁移分支已与 `main` 合并，保留了 `main` 上的导航栏变更；生产发布从 Vercel/Railway 双平台 gate 改为单一 OpenNext Worker 流程（`pnpm cf:deploy:prod` + R2 cache populate + Worker 公共合同 smoke）。旧 deploy gate、Go/Railway runbook 与已移除 API 的 smoke 检查已删除；当前路由所有权见 `docs/operations/worker-route-ownership.md`。GitHub `production` environment 仍须配置 `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID` 及三项 `SMOKE_*` 变量后，自动发布才会放行。
- 2026-08-29：**🎉 Turso→D1 数据迁移完成，生产已切换**：245k+ 行 / 31 表全量入 D1；行数对账全吻合；最大 10 行逐字节一致；生产部署后实测读（octocat indexed）+写（kennethreitz 现场扫描 95.1 分直接落 D1）双通。迁移管线踩坑实录（都已固化在 tmp/turso-dump.mts）：① D1 拒超长语句→60KB 自适应分批；② 含裸换行的 markdown 触发导入管线切分歧义→hex-cast；③ 单列不超限但多列合计超限→20KB 瘦身阈值；④ **JS UTF-16 长度 vs SQLite 字符计数不一致导致分块重建丢 7.7% 字节**→全字节域重建（BLOB 拼接+字节长度守卫，重放安全）；⑤ wrangler OAuth 瞬断→三次重试；后台任务 590s 上限→mv-done 断点续跑 + nohup 脱管构建。**当前架构**：D1（数据）+ Upstash（缓存/锁，待阶段3b 迁 KV/DO）+ R2（ISR）。**Turso 保留 3 天作回滚备份后注销**；回滚=重部署上一版本（binding 消失自动回 Turso）。切换窗口内写入 Turso 的少量数据（约 1h 漂移）滞留旧库，用户已明确接受；受影响用户重扫即自愈。
- 2026-08-29：**🏁 阶段 2 代码全部完成（bfb3b4b）——Go 后端搬迁彻底收官，代码库零 Railway 引用**：B5b=展示层全本地化（profile/vs 移植 27K Go 装配、projects/boards、developers/leaderboard/sitemap 助手直连 lib；三处残余 cookie 转发消费者本地化；go-backend.server.ts 删除；next.config rewrite 机器整体拆除）。B5c=admin 回填五路由恢复、scan force=1 移植、9 个 Go 漂移全处置。openapi 改回同步合同（202/jobs 流程删除）；所有权测试重写为"路由注册表"。856 测试全绿；dev 全站冒烟 10/10（sitemap 2.5MB 全量清单本地出）。**已知小疑点**：/leaderboard RSC 负载 401KB（3 视图×500 条 flight 数据，非渲染回归，阶段 3 顺手瘦身）。**剩余动作**：① 生产切流（部署 production env 即完成——所有批次一次上）+ 用户真实登录验证会话兼容；② Railway 观察 2-3 天下线（deploy-gate 的 Railway 段、Dockerfile.backend、internal/backend/ Go 代码届时一并删除）；③ 然后阶段 3 搬数据。
