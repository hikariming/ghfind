...
Five explicit `cf_d1_r2` registries are accepted. HTTP/read contract stays 1; storage writer compatibility is versioned separately:
...
| 0001–0012     | `cf-d1-feed-12` | 47 / 354                  | 7               | 2                       |
...
`metadata.source.schemaVersion` explicitly selects the reviewed migration set. Schemas 9, 10, 11 and 12 record `runtimeControlSchemaVersion:7`; these migration sets do not advance `feed_runtime_control.schema_version`. The validator requires the actual value 7; changing it to a migration number to match a filename rejects. A control value of 7 alone cannot identify which migration set was installed. Metadata, registry fingerprint and complete table/column descriptors must agree.
...
Schema 12 adds 39 legacy-writer triggers and two adapter control tables. Its migration hash is `26bd945ec52baab00e4307f81258aa39dfef9dea75d979deefbe286c5cda0182`. The snapshot requires `feed_adapter_write_fence` to contain exactly `{id:1,enabled:1}`; a freshly installed but unactivated fence (`enabled=0`) is not a recoverable artifact. `feed_adapter_write_context` must have zero rows and cannot be imported. Restore initializes/retains the enabled fence independently and uses only atomic, transient markers for authorized writes. It must never restore source authorization state or enable legacy writes. Historical schemas 7/9/10/11 and their hashes remain unchanged; accepting historical bytes grants no permission to restore an old writer.
...
```sh
node scripts/feed-snapshot.mjs schema 12
...
The current drill uses schema 12: it verifies all twelve registered migration hashes, installs those statements in real local D1, and compares `PRAGMA table_info`, primary keys, complete/partial unique keys and physical types to the registry. Restoration includes all 354 physical columns without coercion or default substitution. This independently tests schema 12 with storage writer 2 and runtime control 7; historical schema-7/9/11 reports are not reused as schema-12 evidence. Wrangler’s public `unstable_splitSqlQuery` preserves compound trigger bodies, quoted semicolons and comments after exact migration hashes match; real D1 execution validates the SQL. It exposes no caller-supplied SQL interface. SQLite/D1 internal tables and migration bookkeeping are outside the business snapshot. The actual `foreign_key_list` determines insertion order; constraints remain enabled. Installation seeds are removed only from the newly allocated target; the singleton writer control remains closed and the legacy-writer fence remains enabled throughout restoration. The authorization context is never exported/imported. Fixed raw legacy writes are rejected before and after restore, while authorized fixture/adapter batches leave zero committed markers. Disabled-fence and context imports are rejected. A deliberately failing two-row import batch verifies that D1 rolls back the first insert when the second conflicts.
