...
This opt-in harness extends the earlier [startup/idle Docker evidence](../evidence/feed-portability-a3c3f84/README.md) with a real, bounded business-HTTP execution. The [retained evidence](../evidence/feed-docker-http/README.md) includes both the initial setup failure at `565829abc21843bb20ad2a5f3b220d4124c2329f` and the passing corrected run at `39def663ab662e66e0b3473dfc06285e761c2a26`. The first attempt reached PostgreSQL's temporary Unix initialization server and failed before migrations, API-container creation or business HTTP. After authenticated TCP readiness was added, the second attempt passed 21 HTTP calls in 0.657723959 seconds against the standard API image. Original artifacts and binary hashes were independently verified; a separate readback confirmed absence of all nine owned resource identities across both attempts. This accepts the bounded contract described below, not complete portable core E2E or completed deletion cleanup.
...
PostgreSQL readiness uses at most 40 attempts of authenticated TCP `psql -h 127.0.0.1 -p 5432 -d feed_portability_test -c 'SELECT 1'`, with the fixed local synthetic password, a two-second connection timeout, one-second statement timeout and three-second subprocess limit. Both exit zero and the single result `1` are required. The entrypoint temporary initialization server listens only on its Unix socket; accepting connections there is not proof that the final server or requested database is ready. The marker write is attempted once after readiness, without a retry.
