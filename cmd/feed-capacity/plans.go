...
func plans(ctx context.Context, db *sql.DB, out string) error {
	...
	queries["high_user_impression_hit"] = `SELECT MAX(occurred_at) FROM feed.events WHERE github_id=$1 AND repo_key='capacity-owner-00001/synthetic-project-00001' AND event_type='impression'`
